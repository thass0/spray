...
SprayResult
pt_write_memory (pid_t pid, real_addr addr, uint64_t write)
{
...
SprayResult
pt_write_registers (pid_t pid, struct user_regs_struct *regs)
{
  assert (regs != NULL);
...
SprayResult
pt_continue_execution (pid_t pid)
{
...
SprayResult
pt_get_signal_info (pid_t pid, siginfo_t *siginfo)
{
  assert (siginfo != NULL);
