...
const char *
what_dwarf_result (int dwarf_res)
{
  switch (dwarf_res)
    {
    case DW_DLV_OK:
...
Dwarf_Debug
sd_dwarf_init (const char *restrict filepath, Dwarf_Error *error)
{
  ...
  while (1)
      ...
      else if (res == DW_DLV_OK)
	{
	  ...
	  res = sd_search_dwarf_die (dbg,
				     ...
				     level + 1,
				     search_callback,
				     search_for_data, search_findings_data);
...
int
sd_get_high_and_low_pc (Dwarf_Die die,
			Dwarf_Error *error,
			Dwarf_Addr *lowpc, Dwarf_Addr *highpc)
{
  ...
  res = dwarf_highpc_b (die, &highpc_buf, &high_form, &high_class, error);
  ...
  if (high_form != DW_FORM_addr && !dwarf_addr_form_is_indexed (high_form))
    {
...
bool
sd_has_tag (Dwarf_Debug dbg, Dwarf_Die die, Dwarf_Half tagnum)
{
  assert (dbg != NULL);
  assert (dbg != NULL);
...
bool
callback__is_valid_compiler (Dwarf_Debug dbg,
			     Dwarf_Die die,
			     SearchFor search_for,
			     SearchFindings search_findings)
{
...
bool
sd_is_valid_compiler (Dwarf_Debug dbg)
{
  assert (dbg != NULL);
...
void
sd_free_line_table (LineTable *line_table)
{
...
int
sd_line_entry_from_dwarf_line (Dwarf_Line line,
			       LineEntry *line_entry, Dwarf_Error *error)
{
  assert (line != NULL);
  assert (error != NULL);
  ...
  res = dwarf_lineoff_b (line, &colno, error);
...
bool
sd_has_at (Dwarf_Debug dbg, Dwarf_Die die, Dwarf_Half attrnum)
{
  assert (dbg != NULL);
  assert (dbg != NULL);
...
char *
sd_get_filepath (Dwarf_Debug dbg, Dwarf_Die die)
{
  assert (dbg != NULL);
  assert (die != NULL);
...
bool
callback__find_filepath_by_pc (Dwarf_Debug dbg,
			       Dwarf_Die die,
			       SearchFor search_for,
			       SearchFindings search_findings)
{
...
char *
sd_filepath_from_pc (Dwarf_Debug dbg, dbg_addr pc)
{
  assert (dbg != NULL);
  Dwarf_Error error = NULL;
...
bool
sd_is_subprog_with_name (Dwarf_Debug dbg, Dwarf_Die die, const char *name)
{
  assert (dbg != NULL);
  assert (die != NULL);
  assert (name != NULL);
  ...
  if (die_tag == DW_TAG_subprogram)
    {
      ...
      res = dwarf_die_text (die, DW_AT_name, &fn_name_buf, &error);
...
bool
callback_find_subprog_loc_attr_by_subprog_name (Dwarf_Debug dbg,
						Dwarf_Die die,
						SearchFor search_for,
						SearchFindings
						search_findings)
...
sd_get_subprog_loc_attr (Dwarf_Debug dbg,
			 ...
			 Dwarf_Half attr_num, SdLocattr *loc_dest)
{
  assert (dbg != NULL);
...
sd_type_die (Dwarf_Debug dbg, Dwarf_Die die, Dwarf_Die *type_die)
{
  ...
  res = dwarf_dietype_offset (die, &type_off, &type_off_is_info, &error);
  ...
  res = dwarf_offdie_b (dbg,
			type_off, type_off_is_info, &type_die_buf, &error);
  ...
  if (type->n_nodes + 1 >= type->n_alloc)
    {
      ...
      type->nodes =
	realloc (type->nodes, sizeof (*type->nodes) * type->n_alloc);
      if (type->nodes == NULL)
	{
	  return NULL;
	}
  ...
  if (var_search_for->use_scope)
      ...
      if (sd_has_tag (dbg, die, DW_TAG_subprogram))
	{
	  ...
	  res = sd_check_pc_in_die (die,
				    &error,
				    pc, &var_search_findings->in_scope);
	  if (res == DW_DLV_OK)
	    {
...
SprayResult
sd_runtime_variable (Dwarf_Debug dbg,
		     dbg_addr pc,
		     const char *var_name,
		     SdVarattr *attr, char **decl_file, unsigned *decl_line)
{
  assert (dbg != NULL);
...
sd_init_loc_expression (Dwarf_Locdesc_c locdesc_entry,
			...
			SdLocdesc *expr_dest, Dwarf_Error *error)
{
...
const char *
sd_what_operator (SdOperator opcode)
{
...
sd_init_loc_range (Dwarf_Bool debug_addr_missing,
		   Dwarf_Addr lowpc,
		   Dwarf_Addr highpc, SdLocRange *range_dest)
{
...
SprayResult
sd_init_loclist (Dwarf_Debug dbg, SdLocattr loc_attr, SdLoclist *loclist)
{
  assert (dbg != NULL);
  assert (loclist != NULL);
  ...
  if (res != DW_DLV_OK)
      ...
      for (Dwarf_Unsigned i = 0; i < loclist_count; i++)
	{
	  ...
	  if (res != DW_DLV_OK)
	    ...
	    {
	      sd_init_loc_range (debug_addr_missing, low_pc, high_pc,
				 &ranges[i]);
...
void
print_loclist (SdLoclist loclist)
{
  for (size_t i = 0; i < loclist.n_exprs; i++)
    {
      ...
      if (range->meaningful)
	{
	  ...
	  printf ("PC: [0x%lx, 0x%lx) ", range->lowpc.value,
		  range->highpc.value);
	  n_range_chars =
	    n_digits (range->lowpc.value) + n_digits (range->highpc.value) +
	    13;
	}
...
SdLocation
sd_loc_addr (real_addr addr)
{
  return (SdLocation)
  {
  .tag = LOC_ADDR,.addr = addr,};
}
...
void
push_eval_stack (LocEvalStack *stack, SdStackElem push)
{
  assert (stack != NULL);
  ...
  if (stack->sp >= stack->n_alloc)
    {
      ...
      stack->buf =
	realloc (stack->buf, sizeof (*stack->buf) * stack->n_alloc);
      assert (stack->buf != NULL);
    }
}

void
del_eval_stack (LocEvalStack *stack)
{
  if (stack != NULL)
    {
      free (stack->buf);
...
SprayResult
sd_eval_op_fbreg (Dwarf_Debug dbg,
		  SdLocEvalCtx ctx, SdOperation self, LocEvalStack *stack)
{
  ...
  SprayResult res =
    sd_get_subprog_loc_attr (dbg, subprog_name, DW_AT_frame_base, &loc_attr);
  if (res == SP_ERR)
    {
      return SP_ERR;
    }
  ...
  if (location.tag == LOC_REG)
      ...
      else
	{
	  ...
	  uint64_t loc =
	    (uint64_t) ((int64_t) base_addr + (int64_t) self.operand1);
...
SprayResult
sd_eval_op_regn (Dwarf_Debug dbg,
		 SdLocEvalCtx ctx, SdOperation self, LocEvalStack *stack)
{
...
SprayResult
sd_eval_op_addr (Dwarf_Debug dbg,
		 SdLocEvalCtx ctx, SdOperation self, LocEvalStack *stack)
{
  assert (stack != NULL);
  unused (dbg);
...
typedef SprayResult (*SdEvalOp) (Dwarf_Debug dbg,
				 SdLocEvalCtx ctx,
				 SdOperation self, LocEvalStack * stack);

SdEvalOp
sd_get_eval_handler (SdOperator opcode)
{
...
SprayResult
sd_eval_locop (Dwarf_Debug dbg,
	       SdLocEvalCtx ctx, SdOperation op, LocEvalStack *stack)
{
  assert (dbg != NULL);
  assert (stack != NULL);
...
SprayResult
sd_eval_locexpr (Dwarf_Debug dbg,
		 SdLocEvalCtx ctx, SdLocdesc locexpr, SdLocation *location)
{
  ...
  for (size_t i = 0; i < locexpr.n_operations; i++)
    {
      SprayResult res =
	sd_eval_locop (dbg, ctx, locexpr.operations[i], &stack);
      if (res == SP_ERR)
	{
...
SprayResult
sd_eval_loclist (Dwarf_Debug dbg,
...
bool
sd_is_die_from_file (Dwarf_Debug dbg, Dwarf_Die die, const char *filepath)
{
  assert (dbg != NULL);
  assert (die != NULL);
  assert (filepath != NULL);
...
sd_get_filepaths (Dwarf_Debug dbg)
{
  ...
  if (res == DW_DLV_ERROR)
  ...
  else
    {
      ...
      char **filepaths_arr =
	realloc (filepaths.filepaths, sizeof (char *) * (filepaths.idx + 1));
...
LineTable
sd_get_line_table (Dwarf_Debug dbg, const char *filepath)
{
  assert (dbg != NULL);
  assert (filepath != NULL);
			    ...
			    dbg_addr pc, unsigned *index_dest)
{
  ...
  unsigned i = 0;
  while (i < line_table.n_lines && line_table.lines[i].addr.value < pc.value)
    i++;

  if (i < line_table.n_lines &&
...
SprayResult
get_line_table_index_of_line (const LineTable line_table,
			      unsigned lineno, unsigned *index_dest)
{
  assert (index_dest != NULL);

  unsigned i = 0;
  while (i < line_table.n_lines && line_table.lines[i].ln < lineno)
    i++;

  if (i < line_table.n_lines && line_table.lines[i].ln >= lineno)
    {
      *index_dest = i;
      return SP_OK;
    }
...
LineEntry
sd_line_entry_from_pc (Dwarf_Debug dbg, dbg_addr pc)
{
  assert (dbg != NULL);
...
LineEntry
sd_line_entry_at (Dwarf_Debug dbg, const char *filepath, unsigned lineno)
{
  assert (dbg != NULL);
  assert (filepath != NULL);
  ...
  unsigned line_idx = 0;
  SprayResult res =
    get_line_table_index_of_line (line_table, lineno, &line_idx);
  if (res == SP_ERR)
    {
      sd_free_line_table (&line_table);
      return (LineEntry)
...
SubprogPcRange
sd_get_subprog_pc_range (Dwarf_Debug dbg, const char *fn_name)
{
  assert (dbg != NULL);
  assert (fn_name != NULL);
...
SprayResult
sd_for_each_line (Dwarf_Debug dbg, const char *fn_name,
		  const char *filepath,
		  LineCallback callback, void *const init_data)
{
  ...
  for (;
       ...
       line_table.lines[i].addr.value <= attr.highpc.value; i++)
    {
...
SprayResult
sd_effective_start_addr (Dwarf_Debug dbg,
			 dbg_addr prologue_start,
			 dbg_addr function_end, dbg_addr *function_start)
{
  assert (dbg != NULL);
  ...
  for (size_t j = 0; filepaths[j] != NULL; j++)
      ...
      else
	{
	  ...
	  SprayResult res =
	    get_line_table_index_of_pc (line_table, prologue_start,
					&first_line);
	  if (res == SP_ERR)
	    {
	      sd_free_line_table (&line_table);
	    ...
	    {			/* We've found the correct function. */
