...
ElfParseResult parse_elf_header(const Elf64_Ehdr *elf_src,
  ...
  if (
    elf_src->e_phentsize != sizeof(Elf64_Phdr) ||
...
SprayResult file_size(int fd, size_t *dest) {
  assert(dest != NULL);
...
static inline Elf64_Phdr *phdr_at(byte *bytes, size_t off) {
...
const Elf64_Sym *se_symbol_from_name(const char *name, const ElfFile *elf) {
  ...
  const Elf64_Sym *symtab = symtab_at(elf->data.bytes, symtab_hdr->sh_offset);
