...
SprayResult
callback__store_line (LineEntry *line, void *const void_data)
{
  assert (line != NULL);
...
TEST (iterating_lines_works)
{
  ...
  sd_for_each_line (dbg, "main", filepath, callback__store_line, &lines);
  dwarf_finish (dbg);
  free (filepath);
...
bool
callback__test_search (Dwarf_Debug dbg,
		       Dwarf_Die die,
...
TEST (search_returns_the_correct_result)
{
  ...
  res = sd_search_dwarf_dbg (dbg,
			     ...
			     "this_function_name_does_not_exist", NULL);
  assert_int (res, ==, DW_DLV_NO_ENTRY);

  unsigned found_at_level = -1;	/* Not a valid level. */
  res = sd_search_dwarf_dbg (dbg, &error, callback__test_search, "main",	// <- This does exist.
			     &found_at_level);
...
SprayResult
test_get_effective_start_addr (Dwarf_Debug dbg,
			       const DebugSymbol *sym, dbg_addr *dest)
{
  return sd_effective_start_addr (dbg, sym_start_addr (sym),
				  sym_end_addr (sym), dest);
}
...
TEST (finding_variable_declration_files_works)
{
  ...
  char *blah_int1_file =
    realpath ("tests/assets/extern-variables/first_file.c", NULL);
  char *blah_int2_file =
    realpath ("tests/assets/extern-variables/second_file.c", NULL);
  char *blah_int_another_file =
    realpath ("tests/assets/extern-variables/third_file.c", NULL);
  char *my_own_int_file =
    realpath ("tests/assets/extern-variables/main.c", NULL);
  ...
  ASSERT_LOCDESC ("blah_int1", addr, DW_OP_addr, 0x404014, 0, 0, 0, 0,
		  blah_int1_file);
  ASSERT_LOCDESC ("blah_int2", addr, DW_OP_addr, 0x404010, 0, 0, 0, 0,
		  blah_int2_file);
  ASSERT_LOCDESC ("blah_int_another", addr, DW_OP_addr, 0x404018, 0, 0, 0, 0,
		  blah_int_another_file);
  ASSERT_LOCDESC ("my_own_int", addr, DW_OP_addr, 0x40400c, 0, 0, 0, 0,
		  my_own_int_file);
...
TEST (manual_check_locexpr_output)
{
  ...
  {
  .meaningful = true,.lowpc = { 78 },.highpc = { 910 },};
  loclist.ranges[1] = (SdLocRange)
  {
  .meaningful = true,.lowpc = { 11 },.highpc = { 12 },};
